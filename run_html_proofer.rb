...
html_proofer = HTMLProofer.check_directory(
  "build",
  {
    ...
    :ignore_status_codes=>[401,403, 0, 200], # We often link out to the ISO spec so we want to check those links are still there (e.e not 404), but the fact we can't acess them is ok
    :ignore_files => [
